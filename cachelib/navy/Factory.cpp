...
class ZoneCacheProtoImpl final : public ZoneCacheProto {
 public:
  ...
  void setLruEvictionPolicy() override {
    ...
    if (config_.znsConfig.getZLRU()) {
      config_.evictionPolicy = std::make_unique<ZLruPolicy>(numRegions);
      XLOG(INFO, "enable zLRU feature");
    } else {
      config_.evictionPolicy = std::make_unique<LruPolicy>(numRegions);
    }
  }
  ...
  void setZnsConfig(ZnsConfig config) override {
