...
class ZonedDevice final : public Device {
 public:
  ...
  void setResetCallBack(
      std::function<bool(uint64_t, uint64_t, BufferView view)> callback) {
    resetCallback_ = std::move(callback);
    useBottomUpEvicion = true;
  }
  ...
  void setCleanCallBack(
      std::function<bool(uint32_t)> callback) {
    // clean the nr evicted
    cleanZoneCallback_ = std::move(callback);
  }
 ...
 private:
  ...
  std::shared_ptr<DataBlockInfo> dropBlock(const DataBlockInfo &info, std::shared_ptr<Zone> from, std::shared_ptr<Zone> to);
  ...
  bool useCVWait_ = false;
  std::condition_variable zoneStateCV_;
  ...
  std::function<bool(uint64_t, uint64_t, BufferView view)> resetCallback_;
  std::function<bool(uint64_t, uint64_t)> writeCallback_;
  std::function<bool(uint32_t)> cleanZoneCallback_;
  bool useBottomUpEvicion = false;
};
