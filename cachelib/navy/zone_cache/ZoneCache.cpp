...
constexpr uint32_t ZoneCache::kMinAllocAlignSize;
constexpr uint32_t ZoneCache::kMaxItemSize;
constexpr uint32_t ZoneCache::kFormatVersion;
constexpr uint32_t ZoneCache::kDefReadBufferSize;
constexpr uint16_t ZoneCache::kDefaultItemPriority;

ZoneCache::Config& ZoneCache::Config::validate() {
...
void ZoneCache::validate(ZoneCache::Config& config) const {
...
uint32_t ZoneCache::calcAllocAlignSize() const {
...
ZoneCache::ZoneCache(Config&& config)
    : ZoneCache{std::move(config.validate()), ValidConfigTag{}} {}

ZoneCache::ZoneCache(Config&& config, ValidConfigTag)
    : config_{serializeConfig(config)},
      ...
      regionManager_{config.getNumRegions(),
                     ...
                     *config.scheduler,
                     bindThis(&ZoneCache::onRegionReclaim, *this),
                     bindThis(&ZoneCache::onRegionCleanup, *this),
...
std::shared_ptr<BlockCacheReinsertionPolicy> ZoneCache::makeReinsertionPolicy(
...
uint32_t ZoneCache::serializedSize(uint32_t keySize, uint32_t valueSize) {
  uint32_t size = sizeof(EntryDesc) + keySize + valueSize;
  return powTwoAlign(size, allocAlignSize_);
}

Status ZoneCache::insert(HashedKey hk, BufferView value) {
  ...
  if (status == Status::Ok) {
    ...
    // const auto nlr = index_.lookup(hk.keyHash());
    // XLOGF(INFO, "insert: {}", nlr.address());
  }
  allocator_.close(std::move(desc));
  return status;
}

bool ZoneCache::couldExist(HashedKey hk) {
...
Status ZoneCache::lookup(HashedKey hk, Buffer& value) {
  ...
  // XLOGF(DBG, "lookup item hk:{}, addr:{}", hk.keyHash(), lr.address());
  auto addrEnd = decodeRelAddress(lr.address());
  if (!addrEnd.rid().valid()) {
    XLOGF(DBG, "address not found", hk.keyHash(), lr.address());
  ...
  case OpenStatus::Ready: {
    ...
    if (status == Status::Ok) {
      ...
      debugLatencyEstimator_.trackValue(toMicros(getSteadyClock() - timeBegin).count());
    }
...
Status ZoneCache::remove(HashedKey hk) {
  removeCount_.inc();
...
uint32_t ZoneCache::onRegionReclaim(RegionId rid, BufferView buffer) {
  ...
  // XLOGF(DBG, "start reclaim in zone cache {}", rid.index());
  ...
  // XLOGF(DBG, "end reclaim in zone cache {}, cost {} us", rid.index(), toMicros(getSteadyClock() - startTime).count());
  return evictionCount;
}

void ZoneCache::onRegionCleanup(RegionId rid, BufferView buffer) {
...
bool ZoneCache::removeItem(HashedKey hk,
                            uint32_t entrySize,
                            RelAddress currAddr) {
...
ZoneCache::ReinsertionRes ZoneCache::reinsertOrRemoveItem(
    HashedKey hk, BufferView value, uint32_t entrySize, RelAddress currAddr) {
  auto removeItem = [this, hk, entrySize, currAddr] {
    sizeDist_.removeSize(entrySize);
    // XLOGF(DBG, "evict item hk{}, region({}, {})", hk.key(), currAddr.rid().index(), currAddr.offset());
    if (index_.removeIfMatch(hk.keyHash(), encodeRelAddress(currAddr))) {
...
Status ZoneCache::writeEntry(RelAddress addr,
                              uint32_t slotSize,
                              HashedKey hk,
                              BufferView value) {
...
Status ZoneCache::readEntry(const RegionDescriptor& readDesc,
                             RelAddress addr,
                             uint32_t approxSize,
                             HashedKey expected,
                             Buffer& value) {
  ...
  if (desc.csSelf != desc.computeChecksum()) {
    XLOGF(ERR, "error at reading {}, addr.offset: {}", addr.rid().index(), addr.offset());
    lookupEntryHeaderChecksumErrorCount_.inc();
    XCHECK(desc.csSelf == desc.computeChecksum());
    return Status::DeviceError;
  }
...
void ZoneCache::getCounters(const CounterVisitor& visitor) const {
  ...
  debugLatencyEstimator_.visitQuantileEstimator(visitor,
                                               "debug_latency_us");
...
void ZoneCache::persist(RecordWriter& rw) {
...
bool ZoneCache::recover(RecordReader& rr) {
...
void ZoneCache::tryRecover(RecordReader& rr) {
  auto config = deserializeProto<serialization::ZoneCacheConfig>(rr);
...
bool ZoneCache::isValidRecoveryData(
    const serialization::ZoneCacheConfig& recoveredConfig) const {
  ...
  // beyond. Clean this up after ZoneCache everywhere is on v12, and restore
...
serialization::ZoneCacheConfig ZoneCache::serializeConfig(
    const Config& config) {
  serialization::ZoneCacheConfig serializedConfig;
