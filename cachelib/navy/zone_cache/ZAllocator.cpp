...
#include "cachelib/navy/zone_cache/ZAllocator.h"
...
void ZRegionAllocator::setAllocationRegion(RegionId rid) {
  XDCHECK(!rid_.valid());
  rid_ = rid;
}

void ZRegionAllocator::reset() { rid_ = RegionId{}; }

ZAllocator::ZAllocator(ZRegionManager& regionManager, uint16_t numPriorities)
...
std::tuple<RegionDescriptor, uint32_t, RelAddress> ZAllocator::allocate(
    uint32_t size, uint16_t priority) {
  XDCHECK_LT(priority, allocators_.size());
  ZRegionAllocator* ra = &allocators_[priority];
...
// written to the slot.
std::tuple<RegionDescriptor, uint32_t, RelAddress> ZAllocator::allocateWith(
  ...
  if (rid.valid()) {
    ...
    // XLOGF(INFO, "start filling in {}", rid.index());
    // const auto endTime = getSteadyClock();
    // XLOGF(DBG, "fill cost time {}", toMicros(endTime - fillTimes_[rid.index()]).count());
  ...
  XLOGF(DBG, "New Clean Region {}", rid.index());
  ...
  // fillTimes_[rid.index()] = startTime;
...
void ZAllocator::close(RegionDescriptor&& desc) {
  regionManager_.close(std::move(desc));
}

void ZAllocator::flushAndReleaseRegionFromRALocked(ZRegionAllocator& ra,
                                                  bool flushAsync) {
...
void ZAllocator::getCounters(const CounterVisitor& visitor) const {
