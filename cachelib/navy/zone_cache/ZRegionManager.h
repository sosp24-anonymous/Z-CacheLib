...
class ZRegionManager {
 public:
  ...
  //                                  in-mem buffer
  ZRegionManager(uint32_t numRegions,
                uint64_t regionSize,
                uint64_t baseOffset,
                Device& device,
                ...
                const ZnsConfig &config);

  ZRegionManager(const ZRegionManager&) = delete;
  ZRegionManager& operator=(const ZRegionManager&) = delete;
  ...
  void pushTRegion(const uint32_t rid) {
    // XLOGF(INFO, "add region {} to zone {}", rid, zid);
    auto zid = region2zone[rid];
    if (zid != kInvalidIndex) {
      zone2regions[zid].insert(rid);
    }
  }

  void changeTRegion(const uint32_t rid, const uint32_t nzid) {
    // XLOGF(INFO, "add region {} to zone {}", rid, zid);
    auto zid = region2zone[rid];
    if (zid != kInvalidIndex) {
      auto exist = zone2regions[zid].erase(rid);
      if (exist) {
        zone2regions[nzid].insert(rid);
      }
    }
  }

  void deleteTRegion(const uint32_t rid) {
    // XLOGF(INFO, "add region {} to zone {}", rid, zid);
    auto zid = region2zone[rid];
    if (zid != kInvalidIndex) {
      zone2regions[zid].erase(rid);
    }
  }

  void setZone(const uint32_t rid, const uint32_t zid) {
    // XLOGF(INFO, "add region {} to zone {}", rid, zid);
    region2zone[rid] = zid;
    // zone2regions[zid].insert(rid);
  }

  void removeZone(const uint32_t rid) {
    auto zid = region2zone[rid];
    if (zid != kInvalidIndex) {
      // XLOGF(INFO, "remove region {} at zone {}", rid, zid);
      region2zone[rid] = kInvalidIndex;
      // zone2regions[zid].erase(rid);
      nrEvictedRegion_[zid] += 1;
    }
  }
  ...
  void unlockZoneMutex() {
    zoneMutex_.unlock();
  }

  uint32_t getZoneId(const uint32_t rid) {
    // std::lock_guard<std::mutex> lk(zoneMutex_);
    return region2zone[rid];
  }

  uint32_t getNrEvcited(const uint32_t zid) {
    // std::lock_guard<std::mutex> lk(zoneMutex_);
    return nrEvictedRegion_[zid];
  }

  const std::set<uint32_t>& getRegionIds(const uint32_t zid) const {
    return zone2regions[zid];
  }

  std::set<uint32_t>& getRegionIds(const uint32_t zid) {
    return zone2regions[zid];
  }
  ...
  RegionId evictAt(const RegionId &rid);
  ...
  AbsAddress toAbsolute(RelAddress ra) const {
    if (useZoneAsRegion_) {
      return AbsAddress{ra.offset() + ra.rid().index() * zoneOffsetShift_};
    }
  ...
  RelAddress toRelative(AbsAddress aa) const {
    if (useZoneAsRegion_) {
      return RelAddress{RegionId(aa.offset() / zoneOffsetShift_),
                        uint32_t(aa.offset() % zoneOffsetShift_)};
    }
  ...
  bool trimRegion(RelAddress addr) const;
  ...
  // Resets ZRegionManager and recovers region data. Throws std::exception on
  // failure.
  void recover(RecordReader& rr);

  // Exports ZRegionManager stats via CounterVisitor.
  ...
  bool reclaimRegionFromGC(uint64_t offset, uint64_t znsOffset, BufferView view);
 ...
 private:
  ...
  uint64_t physicalOffset(RelAddress addr) const {
    if (useZoneAsRegion_) {
      return toAbsolute(addr).offset();
    }
    return baseOffset_ + toAbsolute(addr).offset();
  }

  RelAddress regionOffset(uint64_t addr) const {
    if (useZoneAsRegion_) {
      return toRelative(AbsAddress(addr));
    }
    return toRelative(AbsAddress(addr - baseOffset_));
  }
  ...
  // direct ZNS
  const bool useZoneAsRegion_;
  // = zone capacity
  const uint64_t zoneOffsetShift_;

  const bool useBottomUpEviction_;

  const bool useZoneAwareLRU_;
  mutable std::mutex zoneMutex_;
  // [region id]
  std::vector<uint32_t> region2zone;
  // [zone id]
  std::vector<std::set<uint32_t>> zone2regions;
  // [zone id]
  std::vector<uint32_t> nrEvictedRegion_;
